...
namespace DFVolume
{
    public class VolumeSampler : MonoBehaviour
    {
        ...
        public float[] GenerateBitmap( out Color[] bitmap)
        {
            // Generate a distance field.
            var df = new float[_resolution * _resolution * _resolution];


            var values = new float[_resolution * _resolution * _resolution *4];
            int id = 0;
            ...
            for (var xi = 0; xi < _resolution; xi++)
            {
                for (var yi = 0; yi < _resolution; yi++)
                {
                    for (var zi = 0; zi < _resolution; zi++)
                    {
                        ...
                        Vector3 v = new Vector3((dx1 - dx0) * dds2,
                                                (dy1 - dy0) * dds2,
                                                (dz1 - dz0) * dds2);
                        ...
                        values[ id ++ ] = d;
                        values[ id ++ ] = v.normalized.x;
                        values[ id ++ ] = v.normalized.y;
                        values[ id ++ ] = v.normalized.z;
                    }
                }
            }
        ...
        float SearchDistance(Vector3 pt)
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                bool hit = Physics.CheckSphere(pt, r);
                //print( hit );
                r += (hit ? -1 : 1) * s;
                s *= 0.5f;
            }
